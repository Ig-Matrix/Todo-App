...
const taskNameLabel = elememtContent("label", "Enter task:");
const taskTimeLabel = elememtContent("label", "Enter time:");
const taskTime = elementProperty("input", "type", "time");
taskTime.setAttribute("placeholder", "Enter a time");
taskTime.required = true;
taskName.required = true;
addTaskBtn = elememtContentAndClassName("button", "add", "addBtn");
...
taskTimeContainer.append(taskTimeLabel, taskTime);
...
function displayTasks(task, time) {
    ...
    const deleteBtn = elememtContentAndClassName(
        "button",
        "Delete",
        "deleteBtn"
    );
...
function displayPopUp(time) {
    ...
    if (userTime === currentDate) {
        const popUpMessage = notif.querySelector(".notif-pop");
